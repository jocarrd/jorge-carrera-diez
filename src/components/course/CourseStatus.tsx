...
function StatusIcon({ status }: { status: LivePart["status"] }) {
  ...
  if (status === "processing") return <span className="course-status-icon is-processing" aria-hidden="true" />;
  ...
  return (
    <section id="estado" className="course-status" aria-labelledby="estado-titulo">
      ...
      <ol className="course-status-parts">
        {parts.map((part) => (
          <li key={part.number} className={`course-status-part is-${part.status}`}>
            <div className="course-status-head">
              ...
              <span className="course-status-date">{formatDay(locale, part.date)}</span>
            </div>
...
/** Aviso corto para la cabecera de cada lección. */
export function LessonSource({ parts, copy, href }: { parts: LivePart[]; copy: CourseCopy; href: string }) {
  const done = parts.filter((p) => p.status === "done").length;
  const pending = parts.some((p) => p.status !== "done");
  if (!pending) return null;
  const text = copy.lessonSource.replace("{done}", String(done)).replace("{total}", String(parts.length));
  return (
