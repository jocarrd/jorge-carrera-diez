...
export type SearchEntry = { kind: "lesson" | "section" | "term"; title: string; context: string; href: string; text: string };
...
/** Busca en títulos de lección, secciones y glosario sin salir de la página. */
export function CourseSearch({ entries, labels }: { entries: SearchEntry[]; labels: { placeholder: string; empty: string; kinds: Record<SearchEntry["kind"], string> } }) {
  ...
  return (
    <div className="course-search" role="search">
      <label className="course-search-field">
        ...
        <input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={labels.placeholder} autoComplete="off" enterKeyHint="search" />
      </label>
      {words.length > 0 ? (
        results.length > 0 ? (
          <ul className="course-search-results" aria-live="polite">
            {results.map((r) => (
              <li key={r.href + r.kind}>
                <Link href={r.href}>
                  <span className="course-search-kind">{labels.kinds[r.kind]}</span>
