  ...
  return (
    <div
      style={{
        ...
        background:
          "radial-gradient(900px 520px at 88% 72%, rgba(251,146,60,0.20), transparent 62%), #0a0a0a",
        color: "#f2f2f2",
        fontFamily: "Geist",
      }}
    >
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div
          style={{
            display: "flex",
            marginTop: 12,
            fontSize: title.length > 34 ? 64 : 76,
            lineHeight: 1.04,
            fontWeight: 600,
            letterSpacing: -2.6,
            maxWidth: 1040,
...
export async function renderLessonOg(locale: Locale, slug: string) {
  ...
  const mod = grokBotCourse.modules.find(
    (m) => m.number === lesson.ref.module,
  )!;
  const moduleName =
    mod.number === 0
      ? copy.introModule
      : `${copy.moduleLabel} ${mod.number} · ${mod.title[locale]}`;
